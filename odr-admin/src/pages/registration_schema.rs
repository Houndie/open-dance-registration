...
pub fn Page(id: ReadOnlySignal<String>) -> Element {
    ...
    let page = use_resource(move || {
        ...
        async move {
            ...
            let event = match response.into_inner().events.pop() {
                Some(event) => event,
                None => {
...
fn LoadedPage(org: ReadOnlySignal<Organization>, event: ReadOnlySignal<proto::Event>) -> Element {
    ...
    let page_body =
        use_resource(move || {
            let mut grpc_client = grpc_client.clone();
            async move {
                let result = grpc_client.registration_schema.query_registration_schemas(
...
fn PageBody(
    ...
    let mut schema: Signal<Schema> = use_signal(move || {
        schema.with(|schema| {
            Schema {
                items: schema.items.iter().map(|item| (Uuid::new_v4(), item.clone())).collect(),
...
#[derive(EnumIter, PartialEq, strum_macros::Display)]
enum ItemFieldsType {
    Text,
    Checkbox,
    Select,
...
fn SchemaItemModal(
    ...
    let success_text = use_memo(move || if initial().id == "" { "Create" } else { "Update" });
    ...
    rsx! {
        div{
            ...
            ondragend: move |_| {
                ...
                let data = match drag_data().as_ref().map(|x| x.clone()) {
                    Some(d) => d,
                    None => return,
                };
